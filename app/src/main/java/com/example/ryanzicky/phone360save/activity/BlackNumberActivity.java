 ...
 * lisview 优化
 * 1.复用convertView
 * 2.对findViewById的使用次数的优化，使用ViewHolder
 * 3.将ViewHolder定义成静态，不去创建多个对象
 * 4.lisview如果有多个条目的时候，可以做分页查询，每一次加载20条，逆序返回
 */
...
//            tv_mode.setText(mBlackNumberList.get(i).mode);
        ...
        lv_blacknumber.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView absListView, int i) {
                ...
                if(i == AbsListView.OnScrollListener.SCROLL_STATE_IDLE
                        ...
                        && !mIsLoad){
                    /* mIsLoad防止重复加载的变量
                    如果当前正在加载mIsLoad就为true，本次加载完毕，再讲mIsLoad改为false
                    如果下一次加载需要去做执行的时候，会判断上诉的mIsLoad变量是否为false，如果为true就需要等待上一次等待完成，将其值改为false后加载*/

                    //如果条目总数大于集合大小的时候才可以继续加载更多
        ...
        bt_submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(!TextUtils.isEmpty(phone)){
                    ...
                    //3.让数据库和集合保持同步（1.数据库中数据重新读一遍，2.向集合中添加一个对象（插入数据构建的对象））
