...
public class AppManagerActivity extends Activity implements View.OnClickListener{
    ...
    private void initTitle() {
        ...
        //3.获取以上两个路径下文件夹的可用大小
        String memoryAcailSpqce = Formatter.formatFileSize(this, getAvailSpace(path));
        String sdMemoryAcailSpqce = Formatter.formatFileSize(this, getAvailSpace(sdPath));

        TextView tv_memory = (TextView) findViewById(R.id.tv_memory);
        TextView tv_sd_memory = (TextView) findViewById(R.id.tv_sd_memory);

        tv_memory.setText("磁盘可用："+memoryAcailSpqce);
        tv_sd_memory.setText("sd卡可用："+sdMemoryAcailSpqce);
    }

    //int代表多少个G

    /**
     * 返回值结果为byte = 8bit，最大结果为 2147483647 bytes
     * @param path
     * @return
     */
